...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 49", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function mafia()
if syrupActive then
moveCodeine = timer.performWithDelay(9000, function (moveZyrp)
...
function passGas()
if fartActive then
moveCreep = timer.performWithDelay(8500, function (creepStuff)
transition.to( creep, { time=800,   x= 400 } )
transition.to( creep, { time=800, delay=1500, x= 700} )
end, 0)
moveGas= timer.performWithDelay(8500, function(gasStuff)
...
function scrollForeground49 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1450 then
		self.x = 1344
...
function scrollBackground49 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1490 then
		self.x = 3960
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground49
		background2.enterFrame= scrollBackground49
		foreground1.enterFrame =scrollForeground49
		foreground2.enterFrame =scrollForeground49
		foreground3.enterFrame =scrollForeground49
		foreground4.enterFrame =scrollForeground49
		timer.resume(tick)
...
sfx.charlieBrown=audio.loadStream("charlieBrown.mp3")
audio.setVolume( .7, { channel=1} )
	function firstPlay()
    sfx.playCharlieBrown=audio.play(sfx.charlieBrown, {channel=1, onComplete = function() 
	...
	background.enterFrame = scrollBackground49 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground49 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground49 --enterframe frame  "hey do this (scrollForeground) thirty times every second
