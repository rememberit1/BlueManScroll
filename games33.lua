...
number = number + 1
textCounter.text = number.."/320  Yds"--change
...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 33", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function mafia()
if syrupActive then
moveCodeine = timer.performWithDelay(12000, function (moveZyrp)
...
function scrollForeground33 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1040 then
		self.x = 500
...
function scrollBackground33 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1000 then
		self.x = 2820
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground33
		background2.enterFrame= scrollBackground33
		foreground1.enterFrame =scrollForeground33
		foreground2.enterFrame =scrollForeground33
		foreground3.enterFrame =scrollForeground33
		foreground4.enterFrame =scrollForeground33
		timer.resume(tick)
...
sfx.sheGot=audio.loadStream("sheGot.mp3")
audio.setVolume( .7, { channel=1} )
	function firstPlay()
   sfx.shePlay=audio.play( sfx.sheGot, {channel=1, onComplete = function() 
   ...
   sfx.shePlay=audio.play( sfx.sheGot, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground33 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground33 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground33 --enterframe frame  "hey do this (scrollForeground33) thirty times every second
