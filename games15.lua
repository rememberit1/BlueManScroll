...
audio.setVolume(.6 ,{hannel=3})	
	...
	thisLevel = display.newText( "Level 15", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function scrollForeground15 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -630 then
		self.x = 500
...
function scrollBackground15 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1000 then
		self.x = 2820
	elseif mollySpeed then 
	self.x = self.x - (self.speed*3)
...
function mscrollForeground15 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -630 then
		self.x = 500
	else self.x = self.x - self.mSpeed
	end
end
function mscrollBackground15 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1000 then
		self.x = 2820
	else self.x = self.x - self.mSpeed
	end
...
if syrupActive then
	...
	if bombActive then 
		...
		background.enterFrame = scrollBackground15
		background2.enterFrame= scrollBackground15
		foreground1.enterFrame =scrollForeground15
		foreground2.enterFrame =scrollForeground15
		foreground3.enterFrame =scrollForeground15
		foreground4.enterFrame =scrollForeground15
		timer.resume(tick)
...
function mollySpeed --change. investigate if this works properly 
	foreground1.enterFrame = scrollForeground152
	Runtime:addEventListener("enterFrame", foreground1)
	foreground2.enterFrame = scrollForeground152
	Runtime:addEventListener("enterFrame", foreground2)
	foreground3.enterFrame = scrollForeground152
	Runtime:addEventListener("enterFrame", foreground3)
	foreground4.enterFrame = scrollForeground152
...
audio.setVolume( .7, { channel=1} )	
	...
	background.enterFrame = scrollBackground15 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground15 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground15 --enterframe frame  "hey do this (scrollForeground15) thirty times every second
	Runtime:addEventListener("enterFrame", foreground1)
	foreground2.enterFrame = scrollForeground15 
	Runtime:addEventListener("enterFrame", foreground2)
	foreground3.enterFrame = scrollForeground15 
