...
number = number + 1
textCounter.text = number.."/220  Yds"--change
...
screenGroup:insert(animation2)
	...
	thisLevel = display.newText( "Level 19", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function scrollForeground19 (self, event)-- self is really just a place holder for cave1, cave2 etc
	if self.x < -250 then
		self.x = 700
...
function scrollForeground19 (self, event)
--if normalSpeed then
	if self.x < -554 then
		self.x = 480
	elseif mollySpeed then 
...
if syrupActive then
	...
	if protectActive then
		...
		background.enterFrame = scrollBackground19
		background2.enterFrame= scrollBackground19
		foreground1.enterFrame =scrollForeground19
		foreground2.enterFrame =scrollForeground19
		foreground3.enterFrame =scrollForeground19
		foreground4.enterFrame =scrollForeground19
		timer.resume(tick)
...
audio.setVolume( .7, { channel=1} )	
	...
	background.enterFrame = scrollBackground19 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground19 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground19 --enterframe frame  "hey do this (scrollForeground19) thirty times every second
