...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 42", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function onEnterFrame( event )
	...
	animation.x = powerupFame.x
	animation.y = powerupFame.y
...
function scrollForeground42 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1450 then
		self.x = 1344
...
function scrollBackground42 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1490 then
		self.x = 3960
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground42
		background2.enterFrame= scrollBackground42
		foreground1.enterFrame =scrollForeground42
		foreground2.enterFrame =scrollForeground42
		foreground3.enterFrame =scrollForeground42
		foreground4.enterFrame =scrollForeground42
		timer.resume(tick)
...
audio.setVolume( .7, { channel=1} )
   ...
   sfx.shePlay=audio.play( sfx.sheGot, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground42 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground42 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground42 --enterframe frame  "hey do this (scrollForeground) thirty times every second
