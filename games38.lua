...
number = number + 1
textCounter.text = number.."/300  Yds"--change
...
--change the background
	background = display.newImage("background/photoshopProjects/after2.png")
	...
	background2 = display.newImage("background/photoshopProjects/after2.png")
	...
	screenGroup:insert(background2)
	 ...
	 foreground1 = display.newImage("background/photoshopProjects/after1.png")
	foreground1:setReferencePoint(display.BottomLeftReferencePoint)
	foreground1.x= -60
	foreground1.y= 350
	foreground1.xScale = .5
	foreground1.yScale = .6
	...
	screenGroup:insert(foreground1)
	 ...
	 foreground2 = display.newImage("background/photoshopProjects/after1.png")
	foreground2:setReferencePoint(display.BottomLeftReferencePoint)
	foreground2.x= 480	
	foreground2.y= 350
	foreground2.xScale = .5
	foreground2.yScale = .6
	foreground2.speed = 2
	...
	twerkingGirlSpriteSheet = sprite.newSpriteSheet("twerkPowerup2.png",154,187)
end
	...
	powerupWrecking.x =1800
	powerupWrecking.y = math.random(125, 175)
	...
	powerupFame.x =700
	powerupFame.y = math.random(125, 175)
	...
	powerupProtect.x =2700
	powerupProtect.y = math.random(135, 195)
	...
	screenGroup:insert (powerupProtect)
		...
		allPowerupSprite7 = sprite.newSpriteSet(allPowerupSpriteSheet6, 1,6)
	sprite.add(allPowerupSprite6, "allPowerups6", 1,6, 1300,0)
	powerupProtect= sprite.newSprite(allPowerupSprite6)--make sure to put at a position where it isnt activated when the counter reaches end of level
	powerupProtect.x =2700
	powerupProtect.y = math.random(135, 195)
	powerupProtect.radius = 10
	powerupProtect.xScale =1.1
	powerupProtect.yScale =.7
	powerupProtect:prepare("allPowerups6")
	powerupProtect:play()
	powerupProtect.speed = 3.5
	...
	bombTarget.xScale =.36
	bombTarget.yScale =.36
	bombTarget.alpha = 0
	bombTarget.x = 130
...
condom = display.newImage("shield.png")
	...
	rum = display.newImage("rum.png")
	rum.xScale =.17
	rum.yScale =.17
	rum.x = 600
	rum.y = 100
	rum.alpha =1
	screenGroup:insert(rum)
	...
	foreground3 = display.newImage("background/photoshopProjects/after0.png")
	foreground3:setReferencePoint(display.BottomLeftReferencePoint)
	foreground3.x= -60
	foreground3.y= 320
	foreground3.xScale = .6
	foreground3.yScale = .4
	foreground3.speed = 7
	screenGroup:insert(foreground3)
	foreground4 = display.newImage("background/photoshopProjects/after0.png")
	foreground4:setReferencePoint(display.BottomLeftReferencePoint)
	foreground4.x= 680
	foreground4.y= 320
	foreground4.xScale = .6
...
sheetData3 = { width=192, height=192, numFrames=25, sheetContentWidth=960, sheetContentHeight=960 }
mySheet3 = graphics.newImageSheet( "burst.png", sheetData3)
sequenceData3 = {start=1, count=25, time=1200, loopCount =1 }
animation3 = display.newSprite( mySheet3, sequenceData3 )
animation3.x = 500 
animation3.y = 500
animation3.alpha =0
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 38", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
	thisLevel:setTextColor( 190, 300, 300 )
	screenGroup:insert(thisLevel)
	
	addedTime = display.newText( "+ 70!", 350, 75, native.systemFont, 28)
...
drunkSound =audio.loadStream("drunkSound.mp3")
audio.setVolume(.2 ,{channel=4})
...
function whole()
if wholeActive then
moveBaby = timer.performWithDelay(11000, function (aBunchOfStuff)
...
moveLegs= timer.performWithDelay(11000, function(OtherStuff)
...
timer.performWithDelay(200, whole, 1)--change

callActive=true
function call()
if callActive then
moveRadio = timer.performWithDelay(11000, function (moveWavvy)
...
moveMan= timer.performWithDelay(11000, function(moveGOP)
...
function mafia()
if syrupActive then
moveCodeine = timer.performWithDelay(10000, function (moveZyrp)
...
function bacardi()
if liqActive then
moveRum = timer.performWithDelay(10000, function (moveWine)
transition.to( rum, { time=1500, x= -100, y=360, rotation=-490 } )
transition.to( rum, { time=1100, delay=1500, x= 800, y=440} )
transition.to( rum, { time=100, delay=2700, x= 800, y=0, rotation=0} )
end, 0)
end
end
timer.performWithDelay(100, bacardi, 1)--change
...
function onEnterFrame( event )
     if jetEngineFiring and (syrupEngineAttached ==false) and (drunkEngineAttached ==false) then
      jet:applyForce (0, -1.4, jet.x, jet.y)
	  
     end
	  if jetEngineFiring and syrupEngineAttached then
      jet:applyForce (0, -1.2, jet.x, jet.y)
     end
	if jetEngineFiring and drunkEngineAttached then
	...
	animation.x = powerupTwerk.x
	animation.y = powerupTwerk.y
	animation2.x = powerupProtect.x
	animation2.y = powerupProtect.y
	animation3.x = powerupMolly.x
	animation3.y = powerupMolly.y
...
function scrollForeground38 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1040 then
		self.x = 500
		elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
		self.rotation =0
		transition.to(self, { time=500, delay=0, rotation = 0} )
		elseif syrupEngineAttached then 
		self.x = self.x-(self.speed/3)
		self.rotation =0
		transition.to(self, { time=500, delay=0, rotation = 0} )		
		elseif drunkEngineAttached then 
		self.x = self.x-self.speed
		--self.rotation =45
		transition.to(self, { time=500, delay=0, rotation = 25} )
	else self.x = self.x - self.speed
	transition.to(self, { time=500, delay=0, rotation = 0} )
	end
end

function scrollBackground38 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1000 then
		self.x = 2820
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
		...
		self.x = self.x-self.speed
		--self.rotation =45
		transition.to(self, { time=500, delay=0, rotation = 30} )
	else self.x = self.x - self.speed
	transition.to(self, { time=500, delay=0, rotation = 0} )
	end
end
...
if sfx.powerGunTrue == 1 then 
		...
		--powerupBomb.enterFrame= pausePowerup
		powerupMolly.enterFrame = pausePowerup
		--powerupFame.enterFrame = pausePowerup
		powerupProtect.enterFrame = pausePowerup
		...
		if drunkEngineAttached then
			timer.pause (drunkTime)

		end
		if liqActive then 
		timer.pause(moveRum)
		transition.pause(rum)
	end
	...
	if callActive then
		timer.pause(moveRadio)
		timer.pause(pulse) 
		timer.pause(moveMan)
		transition.pause( radioWave)
		transition.pause( radioTarget)
		transition.pause( angryMan)
	end
	if fartActive then
		timer.pause(moveCreep)
		timer.pause(moveGas)
		transition.pause(creep)
		transition.pause(fart)
	end
	if syrupActive then
	...
	if bombActive then
		blackBomb.enterFrame = pausePowerup
	end
	if teleportActive then
		phantom.enterFrame = pausePowerup
	end
	if protectActive then
		condom.enterFrame = pausePowerup
	end	
...
if sfx.powerGunTrue == 1 then 
	...
	if fartActive then
		timer.resume(moveCreep)
		timer.resume(moveGas)
	end
	if wholeActive then
	...
	if callActive then 
		timer.resume(moveRadio)
		
		timer.resume(pulse)
		timer.resume(moveMan)
	end
if syrupActive then
	...
	if liqActive then
		timer.resume(moveRum)
	end
	if drunkEngineAttached then 
		timer.resume(drunkTime)
	end
		background.enterFrame = scrollBackground38
		background2.enterFrame= scrollBackground38
		foreground1.enterFrame =scrollForeground38
		foreground2.enterFrame =scrollForeground38
		foreground3.enterFrame =scrollForeground38
		foreground4.enterFrame =scrollForeground38
		timer.resume(tick)
...
function whippingWoman (event)--change and add stuff
	...
	if (whipperSeen and  (hasCollidedCircle(rum, whipTarget))) then
	rum.alpha=.3
	drunkActive = false
	timer.performWithDelay(3000, drunkVisible, 1)
	end
...
return true
 ...
 function leaning (event)
	...
	transition.to( haze, { time=1000, alpha=0, delay=5000 } )
	syrupSpeed =true
 end
 end
 end
 ...
 function afterLeaning ()
	...
	--drunkActive =true
 function youreDrunk (event)
	if (hasCollidedCircle(rum, jet)) then
	if drunkActive then
	if twerkAttached ==false then
	print("alcohol collided")
	rum.alpha =0
	drunkEngineAttached = true
	audio.play(drunkSound, {channel=18})
	drunkTime=timer.performWithDelay(4000, afterDrunk, 1)
	drunkSpeed =true
...
function swingingWreckingBall (event)--change and add stuff
	...
	if (wreckingBallActive and  (hasCollidedCircle(rum, target))) then
	rum.alpha=.3
	drunkActive = false
	timer.performWithDelay(3000, drunkVisible, 1)
	end
...
function fallingBomb (event)--change and add stuff
	...
	if (bombActive and  (hasCollidedCircle(rum, bombTarget))) then
	rum.alpha=.3
	drunkActive = false
	timer.performWithDelay(3000, drunkVisible, 1)
	end
...
function touchScreen (event)
	if jetEngineAttached  then
     if event.phase == "began" then
	  ...
	  machineGunLoop=audio.play(machineGun, {channel=3, loops=5})
     ...
     elseif event.phase == "ended" or event.phase == "cancelled" then
	  ...
	  physics.setGravity(0,-10)
	   ...
	   if  (syrupEngineAttached == false) and (drunkEngineAttached == false) then
	  physics.setGravity(0,12)
	  end
	  if powerupAttached == false then 
...
function slidingProtect (event)--change and add stuff
	...
	if (protectActive and  (hasCollidedCircle(rum, condom))) then
	rum.alpha=.3
	drunkActive = false
	timer.performWithDelay(3000, drunkVisible, 1)
	end
...
sfx.school=audio.loadStream("school.mp3")
audio.setVolume( .7, { channel=1} )
	function firstPlay()
   sfx.schoolPlay=audio.play( sfx.school, {channel=1, onComplete = function() 
   ...
   sfx.schoolPlay=audio.play( sfx.school, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground38 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground38 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground38 --enterframe frame  "hey do this (scrollForeground) thirty times every second
	...
	Runtime:addEventListener("enterFrame", youreDrunk)
...
function scene:exitScene(event) 
	...
	Runtime:removeEventListener("enterFrame", youreDrunk)
