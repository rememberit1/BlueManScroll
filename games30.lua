...
number = number + 1
textCounter.text = number.."/205  Yds"--change
...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 30", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function scrollForeground30 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -650 then
		self.x = 505
...
function scrollBackground30 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1000 then
		self.x = 2820
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground30
		background2.enterFrame= scrollBackground30
		foreground1.enterFrame =scrollForeground30
		foreground2.enterFrame =scrollForeground30
		foreground3.enterFrame =scrollForeground30
		foreground4.enterFrame =scrollForeground30
		timer.resume(tick)
...
sfx.yeahRight=audio.loadStream("yeahRight.mp3")
audio.setVolume( 1, { channel=1} )
	function firstPlay()
   sfx.yeahPlay=audio.play( sfx.yeahRight, {channel=1, onComplete = function() 
   ...
   sfx.yeahPlay=audio.play( sfx.yeahRight, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground30 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground30 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground30 --enterframe frame  "hey do this (scrollForeground30) thirty times every second
