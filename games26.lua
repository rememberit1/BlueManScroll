...
screenGroup:insert(animation2)
	...
	thisLevel = display.newText( "Level 26", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function scrollBackground26 (self, event)-- self is really just a place holder for cave1, cave2 etc
	if self.x < -250 then
		self.x = 700
...
function scrollForeground26 (self, event)
	if self.x < -554 then
		self.x = 480
	elseif mollySpeed then 
	self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if protectActive then
		...
		background.enterFrame = scrollBackground26
		background2.enterFrame= scrollBackground26
		foreground1.enterFrame =scrollForeground26
		foreground2.enterFrame =scrollForeground26
		foreground3.enterFrame =scrollForeground26
		foreground4.enterFrame =scrollForeground26
		timer.resume(tick)
...
audio.setVolume( .7, { channel=1} )
   ...
   sfx.bangingPlay=audio.play( sfx.banging, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground26 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground26 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground26 --enterframe frame  "hey do this (scrollForeground26) thirty times every second
