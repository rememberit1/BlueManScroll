...
number = number + 1
textCounter.text = number.."/410  Yds"--change
...
--change the background
	background = display.newImage("background/dusk.jpg")
	background.xScale = .94
	background.yScale = .64
	background.x = 900
	background.y=100
	background.speed = .4
	screenGroup:insert(background)
	background2 = display.newImage("background/dusk.jpg")
	background2.xScale = .94
	background2.yScale = .64
	background2.x = 2820
	background2.y=100
	background2.speed = .4
	screenGroup:insert(background2)	
	foreground1 = display.newImage("background/duskForeground1.png")
	foreground1:setReferencePoint(display.BottomLeftReferencePoint)
	foreground1.x= -60
	foreground1.y= 300
	foreground1.xScale = .3
...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 36", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function scrollForeground36 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -630 then
		self.x = 500
...
function scrollBackground36 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1000 then
		self.x = 2820
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground36
		background2.enterFrame= scrollBackground36
		foreground1.enterFrame =scrollForeground36
		foreground2.enterFrame =scrollForeground36
		foreground3.enterFrame =scrollForeground36
		foreground4.enterFrame =scrollForeground36
		timer.resume(tick)
...
audio.setVolume( .7, { channel=1} )
   ...
   sfx.shePlay=audio.play( sfx.sheGot, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground36 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground36 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground36 --enterframe frame  "hey do this (scrollForeground36) thirty times every second
