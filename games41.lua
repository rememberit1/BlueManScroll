...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 41", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function scrollForeground41 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1040 then
		self.x = 500
...
function scrollBackground41 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1000 then
		self.x = 2820
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground41
		background2.enterFrame= scrollBackground41
		foreground1.enterFrame =scrollForeground41
		foreground2.enterFrame =scrollForeground41
		foreground3.enterFrame =scrollForeground41
		foreground4.enterFrame =scrollForeground41
		timer.resume(tick)
...
audio.setVolume( .7, { channel=1} )
   ...
   sfx.moneyPlay=audio.play( sfx.money, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground41 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground41 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground41 --enterframe frame  "hey do this (scrollForeground) thirty times every second
