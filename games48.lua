...
end --Game Creator: Ben Akinlosotu
...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 48", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function scrollForeground48 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -950 then
		self.x = 830
...
function scrollBackground48 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1800 then
		self.x = 1770
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground48
		background2.enterFrame= scrollBackground48
		foreground1.enterFrame =scrollForeground48
		foreground2.enterFrame =scrollForeground48
		foreground3.enterFrame =scrollForeground48
		foreground4.enterFrame =scrollForeground48
		timer.resume(tick)
...
audio.setVolume( .7, { channel=1} )
   ...
   sfx.deeDumPlay=audio.play( sfx.deeDum, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground48 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground48 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground48 --enterframe frame  "hey do this (scrollForeground) thirty times every second
