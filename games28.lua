...
number = number + 1
textCounter.text = number.."/355  Yds"--change
...
screenGroup:insert(animation2)
	...
	thisLevel = display.newText( "Level 28", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function scrollForeground28 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1040 then
		self.x = 500
...
function scrollBackground28 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1000 then
		self.x = 2820
	elseif mollySpeed then 
	self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if protectActive then
		...
		background.enterFrame = scrollBackground28
		background2.enterFrame= scrollBackground28
		foreground1.enterFrame =scrollForeground28
		foreground2.enterFrame =scrollForeground28
		foreground3.enterFrame =scrollForeground28
		foreground4.enterFrame =scrollForeground28
		timer.resume(tick)
...
audio.setVolume( .7, { channel=1} )
	...
	background.enterFrame = scrollBackground28 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground28 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground28 --enterframe frame  "hey do this (scrollForeground28) thirty times every second
