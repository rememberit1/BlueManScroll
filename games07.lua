...
step6 = display.newText ("Use Bombs to, ya' know, Blow Stuff Up", 10, 228, native.systemFont, 13)
--step4:setTextColor( 190, 300, 300 )
step6.alpha =0 
screenGroup:insert(step6)
...
screenGroup:insert(animation2)
	...
	thisLevel = display.newText( "Level 7", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
showStep0= timer.performWithDelay(800, function (step4iguess)
step6.alpha =1
end, 1)
showStep1= timer.performWithDelay(4000, function (step5iguess)
step6.alpha =0
end, 1)
end
instructions4()
...
function scrollForeground7 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -650 then
		self.x = 505
...
function scrollBackground7 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1000 then
		self.x = 2820
	else self.x = self.x - self.speed
	end
...
if syrupActive then
	...
	if bombActive then 
		...
		background.enterFrame = scrollBackground7
		background2.enterFrame = scrollBackground7
		foreground1.enterFrame =scrollForeground7
		foreground2.enterFrame =scrollForeground7
		foreground3.enterFrame =scrollForeground7
		foreground4.enterFrame =scrollForeground7
		timer.resume(tick)
...
function whippingWoman (event)--change and add stuff
	...
	audio.fade({channel =1, time=500, volume=1.3}) end, 
	1)
	whip.alpha =1
	jet.alpha=0
	powerupWhip.alpha =0
...
function fallingBomb (event)--change and add stuff
	...
	animation.alpha =1
	animation:play()
	bombGone=timer.performWithDelay(11000, function(something) 
	bombActive =false
	blackBomb.alpha =0
	powerupBomb.alpha =1
	end, 
...
function mollySpeed --change. investigate if this works properly 
	foreground1.enterFrame = scrollForeground72
	Runtime:addEventListener("enterFrame", foreground1)
	foreground2.enterFrame = scrollForeground72
	Runtime:addEventListener("enterFrame", foreground2)
	foreground3.enterFrame = scrollForeground72
	Runtime:addEventListener("enterFrame", foreground3)
	foreground4.enterFrame = scrollForeground72
...
audio.setVolume( 1.3, { channel=1} )	
	...
	background.enterFrame = scrollBackground7 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground7 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground7 --enterframe frame  "hey do this (scrollForeground7) thirty times every second
	Runtime:addEventListener("enterFrame", foreground1)
	foreground2.enterFrame = scrollForeground7 
	Runtime:addEventListener("enterFrame", foreground2)
	foreground3.enterFrame = scrollForeground7 
