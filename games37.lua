...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 37", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function whole()
if wholeActive then
moveBaby = timer.performWithDelay(10000, function (aBunchOfStuff)
...
moveLegs= timer.performWithDelay(10000, function(OtherStuff)
...
function scrollForeground37 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1880 then
		self.x = 1700
...
function scrollBackground37 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1540 then
		self.x = 600
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground37
		background2.enterFrame= scrollBackground37
		foreground1.enterFrame =scrollForeground37
		foreground2.enterFrame =scrollForeground37
		foreground3.enterFrame =scrollForeground37
		foreground4.enterFrame =scrollForeground37
		timer.resume(tick)
...
audio.setVolume( .7, { channel=1} )
   ...
   sfx.kickPlay=audio.play( sfx.kickPause, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground37 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground37 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground37 --enterframe frame  "hey do this (scrollForeground) thirty times every second
