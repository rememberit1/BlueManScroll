...
screenGroup:insert(animation2)
	...
	thisLevel = display.newText( "Level 21", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function scrollForeground21 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -650 then
		self.x = 505
...
function scrollBackground21 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1000 then
		self.x = 2820
	elseif mollySpeed then 
	self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if protectActive then
		...
		background.enterFrame = scrollBackground21
		background2.enterFrame= scrollBackground21
		foreground1.enterFrame =scrollForeground21
		foreground2.enterFrame =scrollForeground21
		foreground3.enterFrame =scrollForeground21
		foreground4.enterFrame =scrollForeground21
		timer.resume(tick)
...
audio.setVolume( .7, { channel=1} )	
	...
	background.enterFrame = scrollBackground21 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground21 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground21 --enterframe frame  "hey do this (scrollForeground21) thirty times every second
