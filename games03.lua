...
number = number + 1
textCounter.text = number.."/210  Yds"--change
...
--change the background
	...
	shootingGun= display.newImage("pistol.png")--change all this
	shootingGun.xScale =.15
	shootingGun.yScale =.15
	shootingGun.alpha = 1
	shootingGun.x = 100
	shootingGun.y = -100
	screenGroup:insert (shootingGun)
...
screenGroup:insert(animation2)
	...
	thisLevel = display.newText( "Level 3", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function  react (event)
		...
		missle1.enterFrame = pausePowerup
		missle2.enterFrame = pausePowerup
		powerupGun.rotation = 0
...
if syrupActive then
		...
		background.enterFrame = scrollBackground3
		background2.enterFrame = scrollBackground3
		foreground1.enterFrame =scrollForeground3
		foreground2.enterFrame =scrollForeground3
		foreground3.enterFrame =scrollForeground3
		foreground4.enterFrame =scrollForeground3
		timer.resume(tick)
...
function scrollForeground3 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -630 then
		self.x = 500
...
function scrollBackground3 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1000 then
		self.x = 2820
	else self.x = self.x - self.speed
	end
...
audio.setVolume( .7, { channel=1} )	
	...
	background.enterFrame = scrollBackground3 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground3 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground3 --enterframe frame  "hey do this (scrollForeground3) thirty times every second
	Runtime:addEventListener("enterFrame", foreground1)
	foreground2.enterFrame = scrollForeground3 
	Runtime:addEventListener("enterFrame", foreground2)
	foreground3.enterFrame = scrollForeground3 
