...
number = number + 1
textCounter.text = number.."/310  Yds"--change
...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 34", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function mafia()
if syrupActive then
moveCodeine = timer.performWithDelay(18000, function (moveZyrp)
...
function scrollForeground34 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1450 then
		self.x = 1344
...
function scrollBackground34 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1490 then
		self.x = 3960
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground34
		background2.enterFrame= scrollBackground34
		foreground1.enterFrame =scrollForeground34
		foreground2.enterFrame =scrollForeground34
		foreground3.enterFrame =scrollForeground34
		foreground4.enterFrame =scrollForeground34
		timer.resume(tick)
...
sfx.andWhat=audio.loadStream("andWhat.mp3")--change
audio.setVolume( .7, { channel=1} )	
	function firstPlay()
    sfx.andPlay=audio.play(sfx.andWhat, {channel=1, onComplete = function() --change twice
	...
	background.enterFrame = scrollBackground34 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground34 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground34 --enterframe frame  "hey do this (scrollForeground34) thirty times every second
