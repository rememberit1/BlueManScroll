...
--change the background
	background = display.newImage("background/photoshopProjects/lit3.jpg")
	...
	background2 = display.newImage("background/photoshopProjects/lit3.jpg")
	...
	screenGroup:insert(background2)
	 ...
	 foreground1 = display.newImage("background/photoshopProjects/lit2.png")
	foreground1:setReferencePoint(display.BottomLeftReferencePoint)
	foreground1.x= -100
	foreground1.y= 350
	foreground1.xScale = .94
	foreground1.yScale = .8
	...
	screenGroup:insert(foreground1)
	 ...
	 foreground2 = display.newImage("background/photoshopProjects/lit2.png")
	foreground2:setReferencePoint(display.BottomLeftReferencePoint)
	foreground2.x= 1690	
	foreground2.y= 350
	foreground2.xScale = .94
	foreground2.yScale = .8
	foreground2.speed = 2
	screenGroup:insert(foreground2)
		
	foreground3 = display.newImage("background/photoshopProjects/lit1.png")
	foreground3:setReferencePoint(display.BottomLeftReferencePoint)
	foreground3.x= -100
	foreground3.y= 320
	foreground3.xScale = .94
	foreground3.yScale = .64
	foreground3.speed = 4
	screenGroup:insert(foreground3)

	foreground4 = display.newImage("background/photoshopProjects/lit1.png")
	foreground4:setReferencePoint(display.BottomLeftReferencePoint)
	foreground4.x= 1695
	foreground4.y= 320
	foreground4.xScale = .94
	foreground4.yScale = .64
	foreground4.speed = 4
...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 50", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function whole()
if wholeActive then
moveBaby = timer.performWithDelay(9000, function (aBunchOfStuff)
...
moveLegs= timer.performWithDelay(9000, function(OtherStuff)
...
function call()
if callActive then
moveRadio = timer.performWithDelay(6000, function (moveWavvy)
...
moveMan= timer.performWithDelay(6000, function(moveGOP)
...
function mafia()
if syrupActive then
moveCodeine = timer.performWithDelay(7000, function (moveZyrp)
...
function bacardi()
if liqActive then
moveRum = timer.performWithDelay(5000, function (moveWine)
...
function passGas()
if fartActive then
moveCreep = timer.performWithDelay(6500, function (creepStuff)
transition.to( creep, { time=800,   x= 400 } )
transition.to( creep, { time=800, delay=1500, x= 700} )
end, 0)
moveGas= timer.performWithDelay(6500, function(gasStuff)
...
function onEnterFrame( event )
	...
	animation3.x = powerupBomb.x
	animation3.y = powerupBomb.y
...
function scrollForeground50 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1880 then
		self.x = 1700
...
function scrollBackground50 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1540 then
		self.x = 600
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground50
		background2.enterFrame= scrollBackground50
		foreground1.enterFrame =scrollForeground50
		foreground2.enterFrame =scrollForeground50
		foreground3.enterFrame =scrollForeground50
		foreground4.enterFrame =scrollForeground50
		timer.resume(tick)
...
sfx.comeHolla=audio.loadStream("comeHolla.mp3")
audio.setVolume( .7, { channel=1} )
	function firstPlay()
   sfx.comeHollaPlay=audio.play( sfx.comeHolla, {channel=1, onComplete = function() 
   ...
   sfx.comeHollaPlay=audio.play( sfx.comeHolla, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground50 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground50 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground50 --enterframe frame  "hey do this (scrollForeground) thirty times every second
