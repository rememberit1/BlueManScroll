...
screenGroup:insert(animation2)
	...
	thisLevel = display.newText( "Level 20", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function passGas()
if fartActive then
moveCreep = timer.performWithDelay(14000, function (creepStuff)
transition.to( creep, { time=800,   x= 400 } )
transition.to( creep, { time=800, delay=1500, x= 700} )
end, 0)
moveGas= timer.performWithDelay(14000, function(gasStuff)
...
function scrollForeground20 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -630 then
		self.x = 500
...
function scrollBackground20 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1000 then
		self.x = 2820
	elseif mollySpeed then 
	self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if protectActive then
		...
		background.enterFrame = scrollBackground20
		background2.enterFrame= scrollBackground20
		foreground1.enterFrame =scrollForeground20
		foreground2.enterFrame =scrollForeground20
		foreground3.enterFrame =scrollForeground20
		foreground4.enterFrame =scrollForeground20
		timer.resume(tick)
...
audio.setVolume( .8, { channel=1} )	
	...
	background.enterFrame = scrollBackground20 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground20 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground20 --enterframe frame  "hey do this (scrollForeground20) thirty times every second
