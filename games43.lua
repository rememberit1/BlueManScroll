...
number = number + 1
textCounter.text = number.."/295  Yds"--change
...
--change the background
	background = display.newImage("background/photoshopProjects/greenCity3.jpg")
	 background.xScale = 1
	background.yScale = .85
	background.x = 0
	background.y=190
	background.speed = .4
	background.alpha =.5
	screenGroup:insert(background)
	background2 = display.newImage("background/photoshopProjects/greenCity3.jpg")
	background2.xScale = 1
	background2.yScale = .85
	background2.x = 1800
	background2.y=190
	background2.speed = .4
	background2.alpha =.5
	screenGroup:insert(background2)
	foreground1 = display.newImage("background/photoshopProjects/greenCity2.png")
	foreground1:setReferencePoint(display.BottomLeftReferencePoint)
	foreground1.x= -100
	foreground1.y= 300
	foreground1.xScale = .5
	foreground1.yScale = .55
	...
	foreground2 = display.newImage("background/photoshopProjects/greenCity2.png")
	foreground2:setReferencePoint(display.BottomLeftReferencePoint)
	foreground2.x= 800	
	foreground2.y= 300
	foreground2.xScale = .5
	foreground2.yScale = .55
	foreground2.speed = 2
	screenGroup:insert(foreground2)
	foreground3 = display.newImage("background/photoshopProjects/greenCity.png")
	foreground3:setReferencePoint(display.BottomLeftReferencePoint)
	foreground3.x= -100
	foreground3.y= 320
	foreground3.xScale = .5
	foreground3.yScale = .2
	foreground3.speed = 5
	screenGroup:insert(foreground3)
	foreground4 = display.newImage("background/photoshopProjects/greenCity.png")
	foreground4:setReferencePoint(display.BottomLeftReferencePoint)
	foreground4.x= 800
	foreground4.y= 320
	foreground4.xScale = .5
	foreground4.yScale = .2
	foreground4.speed = 5
...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 43", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function passGas()
if fartActive then
moveCreep = timer.performWithDelay(11000, function (creepStuff)
transition.to( creep, { time=800,   x= 400 } )
transition.to( creep, { time=800, delay=1500, x= 700} )
end, 0)
moveGas= timer.performWithDelay(11000, function(gasStuff)
...
function scrollForeground43 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -950 then
		self.x = 830
...
function scrollBackground43 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1800 then
		self.x = 1770
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground43
		background2.enterFrame= scrollBackground43
		foreground1.enterFrame =scrollForeground43
		foreground2.enterFrame =scrollForeground43
		foreground3.enterFrame =scrollForeground43
		foreground4.enterFrame =scrollForeground43
		timer.resume(tick)
...
sfx.money=audio.loadStream("money.mp3")
audio.setVolume( .7, { channel=1} )
	function firstPlay()
   sfx.moneyPlay=audio.play( sfx.money, {channel=1, onComplete = function() 
   ...
   sfx.moneyPlay=audio.play( sfx.money, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground43 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground43 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground43 --enterframe frame  "hey do this (scrollForeground) thirty times every second
