...
number = number + 1
textCounter.text = number.."/230  Yds"--change
...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 46", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function bacardi()
if liqActive then
moveRum = timer.performWithDelay(7000, function (moveWine)
...
function onEnterFrame( event )
	...
	animation.x = powerupBomb.x
	animation.y = powerupBomb.y
	animation2.x = powerupMolly.x
	animation2.y = powerupMolly.y
	animation3.x = powerupFame.x
	animation3.y = powerupFame.y
...
function scrollForeground46 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1040 then
		self.x = 500
...
function scrollBackground46 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1000 then
		self.x = 2820
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground46
		background2.enterFrame= scrollBackground46
		foreground1.enterFrame =scrollForeground46
		foreground2.enterFrame =scrollForeground46
		foreground3.enterFrame =scrollForeground46
		foreground4.enterFrame =scrollForeground46
		timer.resume(tick)
...
audio.setVolume( .7, { channel=1} )
   ...
   sfx.comeHollaPlay=audio.play( sfx.comeHolla, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground46 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground46 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground46 --enterframe frame  "hey do this (scrollForeground) thirty times every second
