...
number = number + 1
textCounter.text = number.."/390  Yds"--change
...
screenGroup:insert(animation2)
	...
	thisLevel = display.newText( "Level 24", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
transition.to( codeine, { time=1500, x= -100, y=380, rotation=-490 } )
...
function scrollForeground24 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1220 then
		self.x = 1134
...
function scrollBackground24 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1030 then
		self.x = 2865
	elseif mollySpeed then 
	self.x = self.x - (self.speed*3)
...
function  react (event)
		...
		if syrupEngineAttached or syrupSpeed then
...
if syrupActive then
	...
	if protectActive then
		...
		background.enterFrame = scrollBackground24
		background2.enterFrame= scrollBackground24
		foreground1.enterFrame =scrollForeground24
		foreground2.enterFrame =scrollForeground24
		foreground3.enterFrame =scrollForeground24
		foreground4.enterFrame =scrollForeground24
		timer.resume(tick)
...
audio.setVolume( .7, { channel=1} )	
	...
	background.enterFrame = scrollBackground24 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground24 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground24 --enterframe frame  "hey do this (scrollForeground24) thirty times every second
