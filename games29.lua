...
screenGroup:insert(animation2)
	...
	thisLevel = display.newText( "Level 29", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function bacardi()
if liqActive then
moveRum = timer.performWithDelay(7500, function (moveWine)
...
function scrollForeground29 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1040 then
		self.x = 500
...
function scrollBackground29 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1000 then
		self.x = 2820
		elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
		...
		self.x = self.x-self.speed
		--self.rotation =45
		transition.to(self, { time=500, delay=0, rotation = 35} )
	else self.x = self.x - self.speed
	transition.to(self, { time=500, delay=0, rotation = 0} )
	end
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground29
		background2.enterFrame= scrollBackground29
		foreground1.enterFrame =scrollForeground29
		foreground2.enterFrame =scrollForeground29
		foreground3.enterFrame =scrollForeground29
		foreground4.enterFrame =scrollForeground29
		timer.resume(tick)
...
return true
 ...
 function youreDrunk (event)
	...
	drunkTime=timer.performWithDelay(4500, afterDrunk, 1)
	drunkSpeed =true
	physics.setGravity(0,-12)
...
sfx.allIsRight=audio.loadStream("allIsRight.mp3")--change
audio.setVolume( .7, { channel=1} )	
	function firstPlay()
    sfx.allIsPlay=audio.play(sfx.allIsRight, {channel=1, onComplete = function() --change twice
	...
	background.enterFrame = scrollBackground29 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground29 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground29 --enterframe frame  "hey do this (scrollForeground29) thirty times every second
