...
number = number + 1
textCounter.text = number.."/200  Yds"--change
...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 45", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function onEnterFrame( event )
	...
	animation3.x = powerupWhip.x
	animation3.y = powerupWhip.y
...
function scrollForeground45 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1220 then
		self.x = 1134
...
function scrollBackground45 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1030 then
		self.x = 2865
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground45
		background2.enterFrame= scrollBackground45
		foreground1.enterFrame =scrollForeground45
		foreground2.enterFrame =scrollForeground45
		foreground3.enterFrame =scrollForeground45
		foreground4.enterFrame =scrollForeground45
		timer.resume(tick)
...
audio.setVolume( .7, { channel=1} )
   ...
   sfx.schoolPlay=audio.play( sfx.school, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground45 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground45 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground45 --enterframe frame  "hey do this (scrollForeground) thirty times every second
