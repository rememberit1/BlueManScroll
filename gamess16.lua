...
number = number + 1
textCounter.text = number.."/380  Yds"--change
...
--change the background
	...
	powerupMolly.x =600
	powerupMolly.y = math.random(140, 190)
...
screenGroup:insert(animation2)
	...
	thisLevel = display.newText( "Level 16", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function scrollBackground16 (self, event)--change. self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1190 then
		self.x = 3420
...
function scrollForeground16 (self, event)-- change. self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1030 then
		self.x = 500
	elseif mollySpeed then 
	self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if bombActive then 
		...
		background.enterFrame = scrollBackground16
		background2.enterFrame= scrollBackground16
		foreground1.enterFrame =scrollForeground16
		foreground2.enterFrame =scrollForeground16
		foreground3.enterFrame =scrollForeground16
		foreground4.enterFrame =scrollForeground16
		timer.resume(tick)
...
function ukauka(event)
	if (hasCollidedCircle(powerupTwerk, jet)) then
...
if (hasCollidedCircle(powerupFame, jet)) then
	teleportActive= true
	audio.play(twinkle,{ channel=5})
	audio.fade( { channel=1, time=500, volume=.3 } )
...
function mollySpeed --change. investigate if this works properly 
	foreground1.enterFrame = scrollForeground162
	Runtime:addEventListener("enterFrame", foreground1)
	foreground2.enterFrame = scrollForeground162
	Runtime:addEventListener("enterFrame", foreground2)
	foreground3.enterFrame = scrollForeground162
	Runtime:addEventListener("enterFrame", foreground3)
	foreground4.enterFrame = scrollForeground162
...
audio.setVolume( .7, { channel=1} )
	...
	background.enterFrame = scrollBackground16 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground16 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground16 --enterframe frame  "hey do this (scrollForeground16) thirty times every second
	Runtime:addEventListener("enterFrame", foreground1)
	foreground2.enterFrame = scrollForeground16 
	Runtime:addEventListener("enterFrame", foreground2)
	foreground3.enterFrame = scrollForeground16 
