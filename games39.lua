...
number = number + 1
textCounter.text = number.."/288  Yds"--change
...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 39", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function scrollForeground39 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1880 then
		self.x = 1700
...
function scrollBackground39 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1540 then
		self.x = 600
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground39
		background2.enterFrame= scrollBackground39
		foreground1.enterFrame =scrollForeground39
		foreground2.enterFrame =scrollForeground39
		foreground3.enterFrame =scrollForeground39
		foreground4.enterFrame =scrollForeground39
		timer.resume(tick)
...
sfx.banging=audio.loadStream("banging.mp3")
audio.setVolume( .7, { channel=1} )
	function firstPlay()
   sfx.bangingPlay=audio.play( sfx.banging, {channel=1, onComplete = function() 
   ...
   sfx.bangingPlay=audio.play( sfx.banging, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground39 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground39 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground39 --enterframe frame  "hey do this (scrollForeground) thirty times every second
