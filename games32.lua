...
number = number + 1
textCounter.text = number.."/400  Yds"--change
...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 32", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function scrollForeground32 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1220 then
		self.x = 1134
...
function scrollBackground32 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1030 then
		self.x = 2865
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground32
		background2.enterFrame= scrollBackground32
		foreground1.enterFrame =scrollForeground32
		foreground2.enterFrame =scrollForeground32
		foreground3.enterFrame =scrollForeground32
		foreground4.enterFrame =scrollForeground32
		timer.resume(tick)
...
audio.setVolume( .7, { channel=1} )
   ...
   sfx.bangingPlay=audio.play( sfx.banging, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground32 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground32 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground32 --enterframe frame  "hey do this (scrollForeground32) thirty times every second
