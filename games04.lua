...
screenGroup:insert(animation2)
	...
	thisLevel = display.newText( "Level 4", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
	...
	step5 = display.newText ("Use Target to Whip Enemie's Asses", 10, 228, native.systemFont, 13)
--step4:setTextColor( 190, 300, 300 )
step5.alpha =0 
screenGroup:insert(step5)
...
function scrollForeground4 (self, event)
if normalSpeed then
	if self.x < -554 then
		self.x = 480
...
function scrollBackground4 (self, event)-- self is really just a place holder for cave1, cave2 etc
	if self.x < -250 then
		self.x = 700
	else self.x = self.x - self.speed
	end
...
if syrupActive then
	...
	if protectActive then
		...
		background.enterFrame = scrollBackground4
		background2.enterFrame= scrollBackground4
		foreground1.enterFrame =scrollForeground4
		foreground2.enterFrame =scrollForeground4
		foreground3.enterFrame =scrollForeground4
		foreground4.enterFrame =scrollForeground4
		timer.resume(tick)
...
showStep8= timer.performWithDelay(2800, function (step4iguess)
step5.alpha =1
end, 1)
showStep9= timer.performWithDelay(6000, function (step5iguess)
step5.alpha =0
end, 1)
end
instructions3()
...
sfx.runningMan=audio.loadStream("comeHolla.mp3")--change
audio.setVolume( .7, { channel=1} )
function firstPlay()
...
firstPlay()		
	...
	background.enterFrame = scrollBackground4 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground4 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground4 --enterframe frame  "hey do this (scrollForeground20) thirty times every second
