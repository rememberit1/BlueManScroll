...
number = number + 1
textCounter.text = number.."/340  Yds"--change
...
screenGroup:insert(animation2)
	...
	thisLevel = display.newText( "Level 23", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
	thisLevel:setTextColor( 190, 300, 300 )
	screenGroup:insert(thisLevel)
	
	
	addedTime = display.newText( "+ 70!", 350, 250, native.systemFont, 28)
...
function passGas()
if fartActive then
moveCreep = timer.performWithDelay(12000, function (creepStuff)
transition.to( creep, { time=800,   x= 400 } )
transition.to( creep, { time=800, delay=1500, x= 700} )
end, 0)
moveGas= timer.performWithDelay(12000, function(gasStuff)
...
function scrollBackground23 (self, event)--change. self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1190 then
		self.x = 3420
...
function scrollForeground23 (self, event)-- change. self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1030 then
		self.x = 500
	elseif mollySpeed then 
	self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if protectActive then
		...
		background.enterFrame = scrollBackground23
		background2.enterFrame= scrollBackground23
		foreground1.enterFrame =scrollForeground23
		foreground2.enterFrame =scrollForeground23
		foreground3.enterFrame =scrollForeground23
		foreground4.enterFrame =scrollForeground23
		timer.resume(tick)
...
function moveGhost (self,event)
	...
	if jetEngineAttached == true then
     if event.phase == "began" then
	  ...
	  machineGunLoop=audio.play(machineGun, {channel=3, loops=15})
...
sfx.theStorm=audio.loadStream("theStorm.mp3")--change
audio.setVolume( .7, { channel=1} )	
	function firstPlay()
    sfx.stormPlay=audio.play(sfx.theStorm, {channel=1, onComplete = function() --change twice
	...
	background.enterFrame = scrollBackground23 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground23 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground23 --enterframe frame  "hey do this (scrollForeground23) thirty times every second
