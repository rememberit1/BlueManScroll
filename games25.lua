...
screenGroup:insert(animation2)
	...
	thisLevel = display.newText( "Level 25", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function mafia()
if syrupActive then
moveCodeine = timer.performWithDelay(7500, function (moveZyrp)
...
function scrollForeground25 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1040 then
		self.x = 500
...
function scrollBackground25 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1000 then
		self.x = 2820
	elseif mollySpeed then 
	self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if protectActive then
		...
		background.enterFrame = scrollBackground25
		background2.enterFrame= scrollBackground25
		foreground1.enterFrame =scrollForeground25
		foreground2.enterFrame =scrollForeground25
		foreground3.enterFrame =scrollForeground25
		foreground4.enterFrame =scrollForeground25
		timer.resume(tick)
...
audio.setVolume( 1, { channel=1} )
   ...
   sfx.yeahPlay=audio.play( sfx.yeahRight, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground25 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground25 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground25 --enterframe frame  "hey do this (scrollForeground25) thirty times every second
