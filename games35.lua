...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 35", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function scrollForeground35 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -554 then
		self.x = 480
...
function scrollBackground35 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -250 then
		self.x = 700
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground35
		background2.enterFrame= scrollBackground35
		foreground1.enterFrame =scrollForeground35
		foreground2.enterFrame =scrollForeground35
		foreground3.enterFrame =scrollForeground35
		foreground4.enterFrame =scrollForeground35
		timer.resume(tick)
...
audio.setVolume( .7, { channel=1} )	
	...
	background.enterFrame = scrollBackground35 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground35 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground35 --enterframe frame  "hey do this (scrollForeground35) thirty times every second
