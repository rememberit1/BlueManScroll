...
number = number + 1
textCounter.text = number.."/170  Yds"
...
step1 = display.newText ("Tap/hold anywhere to fly, release to drop, Get to Distance Reached to beat level", 10, 228, 270, 200, native.systemFont, 14)
--step1:setTextColor( 190, 300, 300 )
step1.alpha =0 
screenGroup:insert(step1)

step2 = display.newText ("Avoid enemies (duh!)", 10, 228, native.systemFont, 14)
--step2:setTextColor( 190, 300, 300 )
step2.alpha =0 
screenGroup:insert(step2)

step3 = display.newText ("Crash into box for some crazy shit", 10, 225, native.systemFont, 14)
--step3:setTextColor( 190, 300, 300 )
step3.alpha =0 
screenGroup:insert(step3)
	...
	gameSquare.x = 10
	gameSquare.yScale = .43
	gameSquare.xScale = .24
	gameSquare.y = 187
	...
	thisLevel = display.newText( "Level 1", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
--wreckingSound=audio.loadStream("wreckingSound.mp3")
smallBang=audio.loadSound("smallBang.mp3")
--youFather = audio.loadSound("youAreTheFather.mp3")
backSound =audio.loadSound("beep2.mp3")
audio.setVolume(.6 ,{channel=5})

isFirePlaying = audio.isChannelPlaying( 2 )
...
--timer.performWithDelay(2000, whole, 1)


function instructions ()
showStep1= timer.performWithDelay(100, function (step1iguess)
step1.alpha =1
end, 1)
showStep2= timer.performWithDelay(3000, function (step2iguess)
step2.alpha =1
step1.alpha =0
end, 1)
showStep3= timer.performWithDelay(6000, function (step3iguess)
step3.alpha =1
step2.alpha =0
end, 1)
...
removeInstructions =timer.performWithDelay(9000, function (step4iguess)
step3.alpha =0
end, 1)
end
...
function onCollision2 ()
		...
		explode()
		timer.cancel(tick)
		timer.cancel(removeInstructions)
		end
	end
end
...
function scrollBackground1 (self, event)-- self is really just a place holder for cave1, cave2 etc
	if self.x < -250 then
		self.x = 700
...
function scrollForeground1 (self, event)
if normalSpeed then
	if self.x < -554 then
		self.x = 480
	else self.x = self.x - self.speed	
...
timer.cancel(showStep1)
timer.cancel(showStep2)
timer.cancel(showStep3)
timer.cancel(removeInstructions)
end
end
if mine2Active then
...
function twerkTouchScreen (event)
	...
	if twerkAttached == true then
	jetEngineAttached = false --take this line and move it down also set linearvelocity to (0,0) right below ( 0, 144 )
...
function ukauka(event)
	...
	twerkTime= timer.performWithDelay(9000, normal)
	twerkResumeSound =timer.performWithDelay(8100, function(something) 
...
cant.alpha =1
	...
	function proact (event)
		...
		background.enterFrame = scrollBackground1
		background2.enterFrame = scrollBackground1
		foreground1.enterFrame =scrollForeground1
		foreground2.enterFrame =scrollForeground1
		foreground3.enterFrame =scrollForeground1
		foreground4.enterFrame =scrollForeground1
		timer.resume(tick)
...
function mollySpeed --change. investigate if this works properly 
	foreground1.enterFrame = scrollForeground12
	Runtime:addEventListener("enterFrame", foreground1)
	foreground2.enterFrame = scrollForeground12
	Runtime:addEventListener("enterFrame", foreground2)
	foreground3.enterFrame = scrollForeground12
	Runtime:addEventListener("enterFrame", foreground3)
	foreground4.enterFrame = scrollForeground12
...
audio.setVolume( .6, { channel=4} )	
	...
	foreground1.enterFrame = scrollForeground1 --enterframe frame  "hey do this (scrollForeground1) thirty times every second
	Runtime:addEventListener("enterFrame", foreground1)
	foreground2.enterFrame = scrollForeground1 
	Runtime:addEventListener("enterFrame", foreground2)
	foreground3.enterFrame = scrollForeground1 
