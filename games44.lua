...
--change the background
	background = display.newImage("background/photoshopProjects/angbrd3.png")
	...
	background2 = display.newImage("background/photoshopProjects/angbrd3.png")
	background2.xScale = 1.6
	background2.yScale = 2
	background2.x = 3000
	background2.y=150
	background2.speed = .4
	screenGroup:insert(background2)	
	foreground1 = display.newImage("background/photoshopProjects/angbrd2.png")
	foreground1:setReferencePoint(display.BottomLeftReferencePoint)
	foreground1.x= -60
	foreground1.y= 320
	foreground1.xScale = .8
	foreground1.yScale = 1.2
	...
	foreground2 = display.newImage("background/photoshopProjects/angbrd2.png")
	foreground2:setReferencePoint(display.BottomLeftReferencePoint)
	foreground2.x= 1350	
	foreground2.y= 320
	foreground2.xScale = .8
	foreground2.yScale = 1.2
	foreground2.speed = 2
	screenGroup:insert(foreground2)
	foreground3 = display.newImage("background/photoshopProjects/angbrd1.png")
	foreground3:setReferencePoint(display.BottomLeftReferencePoint)
	foreground3.x= -60
	foreground3.y= 390
	foreground3.xScale = .8
	foreground3.yScale = 1.2
	foreground3.speed = 5
	screenGroup:insert(foreground3)
	foreground4 = display.newImage("background/photoshopProjects/angbrd1.png")
	foreground4:setReferencePoint(display.BottomLeftReferencePoint)
	foreground4.x= 1350
	foreground4.y= 390
	foreground4.xScale = .8
	foreground4.yScale = 1.2
	foreground4.speed = 5
...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 44", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function call()
if callActive then
moveRadio = timer.performWithDelay(7000, function (moveWavvy)
...
moveMan= timer.performWithDelay(7000, function(moveGOP)
...
function bacardi()
if liqActive then
moveRum = timer.performWithDelay(8000, function (moveWine)
...
function onEnterFrame( event )
	...
	animation.x = powerupMolly.x
	animation.y = powerupMolly.y
	animation2.x = powerupBomb.x
	animation2.y = powerupBomb.y
	animation3.x = powerupProtect.x
	animation3.y = powerupProtect.y
...
function scrollForeground44 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1450 then
		self.x = 1344
...
function scrollBackground44 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1490 then
		self.x = 3960
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground44
		background2.enterFrame= scrollBackground44
		foreground1.enterFrame =scrollForeground44
		foreground2.enterFrame =scrollForeground44
		foreground3.enterFrame =scrollForeground44
		foreground4.enterFrame =scrollForeground44
		timer.resume(tick)
...
-- audio.dispose(extacy)
-- audio.dispose(youFather)
-- audio.dispose(fartSound)
-- audio.dispose(syrupSound)
--audio.dispose(drunkSound)
...
sfx.deeDum=audio.loadStream("deeDum.mp3")
audio.setVolume( .7, { channel=1} )
	function firstPlay()
   sfx.deeDumPlay=audio.play( sfx.deeDum, {channel=1, onComplete = function() 
   ...
   sfx.deeDumPlay=audio.play( sfx.deeDum, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground44 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground44 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground44 --enterframe frame  "hey do this (scrollForeground) thirty times every second
