...
number = number + 1
textCounter.text = number.."/260  Yds"--change
...
--change the background
	background = display.newImage("background/photoshopProjects/mourn3.jpg")--change
	background.xScale = 1.17
	background.yScale = .8
	background.x = 500
	background.y=200
	background.speed = .4
	...
	background2 = display.newImage("background/photoshopProjects/mourn3.jpg")
	background2.xScale = 1.17
	...
	foreground1 = display.newImage("background/photoshopProjects/mourn2.png")
	foreground1:setReferencePoint(display.BottomLeftReferencePoint)
	foreground1.x= -60
	foreground1.y= 320
	foreground1.xScale = .4
	foreground1.yScale = .5
	foreground1.speed = 2
	--foreground1.speed2 = 4	--adding(period) .etc to a variable, it is just adding a variable to a variable (changing properties of this variable). speed is is not a corona variable
	screenGroup:insert(foreground1)
	foreground2 = display.newImage("background/photoshopProjects/mourn2.png")
	foreground2:setReferencePoint(display.BottomLeftReferencePoint)
	foreground2.x= 480	
	foreground2.y= 320
	foreground2.xScale = .4
	foreground2.yScale = .5
	foreground2.speed = 2
	--foreground2.speed2 = 4
	screenGroup:insert(foreground2)
	foreground3 = display.newImage("background/photoshopProjects/mourn1.png")
	foreground3:setReferencePoint(display.BottomLeftReferencePoint)
	foreground3.x= -60
	foreground3.y= 320
	foreground3.xScale = .4
	foreground3.yScale = .5
	foreground3.speed = 4
	--foreground3.speed2 = 8
	screenGroup:insert(foreground3)
	foreground4 = display.newImage("background/photoshopProjects/mourn1.png")
	foreground4:setReferencePoint(display.BottomLeftReferencePoint)
	foreground4.x= 680
	foreground4.y= 320
	foreground4.xScale = .4
...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 31", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function whole()
if wholeActive then
moveBaby = timer.performWithDelay(11500, function (aBunchOfStuff)
...
moveLegs= timer.performWithDelay(11500, function(OtherStuff)
...
function call()
if callActive then
moveRadio = timer.performWithDelay(9000, function (moveWavvy)
...
moveMan= timer.performWithDelay(9000, function(moveGOP)
...
function bacardi()
if liqActive then
moveRum = timer.performWithDelay(9000, function (moveWine)
...
function onEnterFrame( event )
	...
	animation.x = powerupProtect.x
	animation.y = powerupProtect.y
	animation2.x = powerupWrecking.x
	animation2.y = powerupWrecking.y
	animation3.y = powerupTwerk.y
	animation3.y = powerupTwerk.y
...
function scrollForeground31 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1030 then
		self.x = 500
...
function scrollBackground31 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1190 then
		self.x = 3420
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground31
		background2.enterFrame= scrollBackground31
		foreground1.enterFrame =scrollForeground31
		foreground2.enterFrame =scrollForeground31
		foreground3.enterFrame =scrollForeground31
		foreground4.enterFrame =scrollForeground31
		timer.resume(tick)
...
return true
 ...
 function youreDrunk (event)
	...
	--transition.to( stars, { time=500, alpha=.6 } )
	--transition.to( stars, { time=1000, alpha=0, delay=4000 } )
	drunkSpeed =true
...
sfx.kickPause=audio.loadStream("kickPause.mp3")
audio.setVolume( .7, { channel=1} )
	function firstPlay()
   sfx.kickPlay=audio.play( sfx.kickPause, {channel=1, onComplete = function() 
   ...
   sfx.kickPlay=audio.play( sfx.kickPause, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground31 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground31 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground31 --enterframe frame  "hey do this (scrollForeground31) thirty times every second
