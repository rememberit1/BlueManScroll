...
number = number + 1
textCounter.text = number.."/370  Yds"--change
...
screenGroup:insert(animation2)
	...
	thisLevel = display.newText( "Level 12", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function scrollBackground12 (self, event)--change. self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1190 then
		self.x = 3420
...
function scrollForeground12 (self, event)-- change. self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1030 then
		self.x = 500
	else self.x = self.x - self.speed
	end
...
if syrupActive then
	...
	if bombActive then 
		...
		background.enterFrame = scrollBackground12
		background2.enterFrame = scrollBackground12
...
function mollySpeed --change. investigate if this works properly 
	foreground1.enterFrame = scrollForeground122
	Runtime:addEventListener("enterFrame", foreground1)
	foreground2.enterFrame = scrollForeground122
	Runtime:addEventListener("enterFrame", foreground2)
	foreground3.enterFrame = scrollForeground122
	Runtime:addEventListener("enterFrame", foreground3)
	foreground4.enterFrame = scrollForeground122
...
audio.setVolume( .7, { channel=1} )	
	...
	firstPlay()	
		...
		--this is song 6
	-- sfx.fastRing=audio.loadStream("fastRing.mp3")--change
-- audio.setVolume( .7, { channel=1} )	
	-- function firstPlay()
    -- sfx.fastPlay=audio.play(sfx.fastRing, {channel=1, onComplete = function() --change twice
									  -- if levelBeaten == false then
									  -- secondPlay()
									  -- end
                                      -- end})
	...
	background.enterFrame = scrollBackground12 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground12 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground12 --enterframe frame  "hey do this (scrollForeground12) thirty times every second
	Runtime:addEventListener("enterFrame", foreground1)
	foreground2.enterFrame = scrollForeground12 
	Runtime:addEventListener("enterFrame", foreground2)
	foreground3.enterFrame = scrollForeground12 
