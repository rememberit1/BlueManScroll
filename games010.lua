...
screenGroup:insert(animation2)
	...
	thisLevel = display.newText( "Level 10", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
audio.setVolume(.3 ,{channel=5})
audio.setVolume(.3 ,{hannel=3})


wholeActive=true
function whole()
if wholeActive then
moveBaby = timer.performWithDelay(13500, function (aBunchOfStuff)
...
moveLegs= timer.performWithDelay(13500, function(OtherStuff)
...
function scrollForeground10 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -630 then
		self.x = 500
...
function scrollBackground10 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1000 then
		self.x = 2820
	elseif mollySpeed then 
	self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if bombActive then 
		...
		background.enterFrame = scrollBackground10
		background2.enterFrame = scrollBackground10
		foreground1.enterFrame =scrollForeground10
		foreground2.enterFrame =scrollForeground10
		foreground3.enterFrame =scrollForeground10
		foreground4.enterFrame =scrollForeground10
		timer.resume(tick)
...
function ukauka(event)
	...
	timer.performWithDelay(7000, normal, 1)
...
function mollySpeed --change. investigate if this works properly 
	foreground1.enterFrame = scrollForeground102
	Runtime:addEventListener("enterFrame", foreground1)
	foreground2.enterFrame = scrollForeground102
	Runtime:addEventListener("enterFrame", foreground2)
	foreground3.enterFrame = scrollForeground102
	Runtime:addEventListener("enterFrame", foreground3)
	foreground4.enterFrame = scrollForeground102
...
sfx.currentLevel =10--change

sfx.kinetic=audio.loadStream("kinetic.mp3")--change
audio.setVolume( .7, { channel=1} )	
	function firstPlay()
    sfx.kineticPlay=audio.play(sfx.kinetic, {channel=1, onComplete = function() --change twice
	...
	background.enterFrame = scrollBackground10 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground10 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground10 --enterframe frame  "hey do this (scrollForeground10) thirty times every second
	Runtime:addEventListener("enterFrame", foreground1)
	foreground2.enterFrame = scrollForeground10 
	Runtime:addEventListener("enterFrame", foreground2)
	foreground3.enterFrame = scrollForeground10 
