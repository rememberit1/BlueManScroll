...
number = number + 1
textCounter.text = number.."/280  Yds"--change
...
screenGroup:insert(animation2)
	...
	thisLevel = display.newText( "Level 22", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function scrollForeground22 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1040 then
		self.x = 500
...
function scrollBackground22 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1000 then
		self.x = 2820
	elseif mollySpeed then 
	self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if protectActive then
		...
		background.enterFrame = scrollBackground22
		background2.enterFrame= scrollBackground22
...
audio.setVolume( .7, { channel=1} )	
	...
	background.enterFrame = scrollBackground22 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground22 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground22 --enterframe frame  "hey do this (scrollForeground22) thirty times every second
