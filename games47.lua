...
number = number + 1
textCounter.text = number.."/240  Yds"--change
...
screenGroup:insert(animation3)
	...
	thisLevel = display.newText( "Level 47", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
function scrollForeground47 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1880 then
		self.x = 1700
...
function scrollBackground47 (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x < -1540 then
		self.x = 600
	elseif mollySpeed then 
		self.x = self.x - (self.speed*3)
...
if syrupActive then
	...
	if drunkEngineAttached then 
		...
		background.enterFrame = scrollBackground47
		background2.enterFrame= scrollBackground47
		foreground1.enterFrame =scrollForeground47
		foreground2.enterFrame =scrollForeground47
		foreground3.enterFrame =scrollForeground47
		foreground4.enterFrame =scrollForeground47
		timer.resume(tick)
...
audio.setVolume( .7, { channel=1} )
   ...
   sfx.comeHollaPlay=audio.play( sfx.comeHolla, {channel= 1, onComplete = function()
	...
	background.enterFrame = scrollBackground47 --change
	Runtime:addEventListener("enterFrame", background)
	background2.enterFrame = scrollBackground47 --change
	Runtime:addEventListener("enterFrame", background2)
	foreground1.enterFrame = scrollForeground47 --enterframe frame  "hey do this (scrollForeground) thirty times every second
