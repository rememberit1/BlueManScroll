...
number = number + 1
textCounter.text = number.."/290  Yds"
...
if (number > 289) then --change. mainmenu: add a "or" item click here
...
screenGroup:insert(animation2)
	...
	thisLevel = display.newText( "Level 13", 15, 65, native.systemFont, 14)
	thisLevel.alpha =.9
...
transition.to( spreadLegs, { time=1000, delay=1200, x= 600, y =180 } )
...
function scrollBackground13 (self, event)-- self is really just a place holder for cave1, cave2 etc
	if self.x < -250 then
		self.x = 700
...
function scrollForeground13 (self, event)
if normalSpeed then
	if self.x < -554 then
		self.x = 480
	else self.x = self.x - self.speed	
...
function moveWhipCrack (self, event)-- self is really just a place holder for foreground1, foreground2 etc
	if self.x > 180 then	 
	 ...
	 if whipperSeen then
	 whipTarget.alpha =1
	 loopWhipSound=timer.performWithDelay(300,audio.play(whipSound, {channel=11}),1)
	 end
...
if syrupActive then
	...
	if bombActive then 
		...
		background.enterFrame = scrollBackground13
		background2.enterFrame= scrollBackground13
		foreground1.enterFrame =scrollForeground13
		foreground2.enterFrame =scrollForeground13
		foreground3.enterFrame =scrollForeground13
		foreground4.enterFrame =scrollForeground13
		timer.resume(tick)
...
function whippingWoman (event)--change and add stuff
	...
	animation2.alpha =1
	animation2:play()
	timer.performWithDelay(8000, normalAfterWhip, 
	1)
	end
...
media.playVideo( "firstMovi.mp4", true, gameOver2)
end
...
jetEngineFiring=false
audio.stop()
-- audio.dispose(sfx.yeahRight)
-- --audio.dispose(fireSong)
-- audio.dispose(whipSound)
...
function mollySpeed --change. investigate if this works properly 
	foreground1.enterFrame = scrollForeground132
	Runtime:addEventListener("enterFrame", foreground1)
	foreground2.enterFrame = scrollForeground132
	Runtime:addEventListener("enterFrame", foreground2)
	foreground3.enterFrame = scrollForeground132
	Runtime:addEventListener("enterFrame", foreground3)
	foreground4.enterFrame = scrollForeground132
...
-- audio.setVolume( 1, { channel=1} )
   ...
   -- sfx.yeahPlay=audio.play( sfx.yeahRight, {channel= 1, onComplete = function()
	...
	foreground1.enterFrame = scrollForeground13 --enterframe frame  "hey do this (scrollForeground13) thirty times every second
	Runtime:addEventListener("enterFrame", foreground1)
	foreground2.enterFrame = scrollForeground13 
	Runtime:addEventListener("enterFrame", foreground2)
	foreground3.enterFrame = scrollForeground13 
